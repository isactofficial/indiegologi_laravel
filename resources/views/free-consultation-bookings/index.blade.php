    ...
    <div class="container-fluid px-4" style="min-height: 100vh;">
        ...
        {{-- <div class="row mb-4">
            <div class="col-md-12 d-flex justify-content-end">
                <a href="{{ route('admin.free-consultation-bookings.create') }}"
        ...
        <div class="card border-0 rounded-4 shadow-sm">
            <div class="card-body p-4">
                @if(session('success'))
                    <div class="alert alert-success rounded-3 mb-4"><i
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse($bookings as $booking)
                                <tr>
                                    <td class="py-3 fw-semibold" style="color: var(--theme-primary);">{{ $booking->user->name }}
                                    </td>
                                    <td class="py-3">{{ optional($booking->invoice)->invoice_no ?? 'N/A' }}</td>
                                    <td class="py-3">
                                        <span>
                                            {{ ucwords($booking->session_type) }}
                                        </span>
                                    </td>
                                    <td class="py-3">
                                        @php
                                            $bookingStatus = strtolower($booking->booking_status ?? 'menunggu pembayaran');
                                            $statusClass = [
                                                'confirmed' => 'bg-success'
                                            ][(string) $bookingStatus];
                                        @endphp
                                        <span
                                            class="badge {{ $statusClass }}">{{ ucfirst(str_replace('_', ' ', $bookingStatus)) }}</span>
                                    </td>
                                    <td class="py-3">
                                    ...
                                    <td class="py-3">
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('admin.free-consultation-bookings.show', $booking->id) }}"
                                                class="btn btn-sm btn-outline-primary rounded-pill px-3"
                                                title="Lihat Detail Booking">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.free-consultation-bookings.edit', $booking->id) }}"
                                                class="btn btn-sm btn-outline-secondary rounded-pill px-3" title="Edit Booking">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('admin.free-consultation-bookings.destroy', $booking->id) }}"
                        ...
                        <div class="booking-card">
                            <div class="booking-card-header">
                                ...
                                <div class="booking-price">
                                    <div class="fw-bold fs-5" style="color: var(--theme-primary);">Rp
                                        {{ $booking->total_price}}
                                    </div>
                                </div>
                            </div>
                            <div class="booking-card-body">
                                <div class="session-status-container">
                                    <span
                                        class="badge badge-status badge-{{ str_replace(' ', '-', strtolower($booking->session_status)) }}">
                            ...
                            <div class="booking-card-footer">
                                @if ($booking->user)
                                    <a href="{{ route('admin.users.show', $booking->user->id) }}"
                                        class="btn btn-sm btn-outline-primary" title="Lihat Profil User"><i
                                            class="fas fa-eye"></i></a>
                                @endif
                                <a href="{{ route('admin.consultation-bookings.show', $booking->id) }}"
                                    class="btn btn-sm btn-outline-info" title="Lihat Invoice"><i
                                        class="fas fa-file-invoice"></i></a>
                                <a href="{{ route('admin.consultation-bookings.edit', $booking->id) }}"
                                    class="btn btn-sm btn-outline-secondary" title="Edit Booking"><i
                                        class="fas fa-edit"></i></a>
                                <form action="{{ route('admin.consultation-bookings.destroy', $booking->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete(event, this.parentElement)"
                                        class="btn btn-sm btn-outline-danger w-100" title="Hapus Booking"><i
